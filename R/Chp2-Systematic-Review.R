...
knitr::kable(agreement.table.1, col.names = c("","","Exclude", "Include","Total"),  caption = 'Inter-rater reliability') %>%
  ...
  add_header_above(c(" " = 2, "Initial screening descision" = 3)) %>%
...
knitr::kable(agreement.table.2, col.names = c("","","Exclude", "Include","Total"),  caption = 'Intra-rater reliability') %>%
  kable_styling("striped") %>%
