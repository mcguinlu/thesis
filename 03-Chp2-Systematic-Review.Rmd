...
How to interpret agreement co-efficients is widely debated. Here we use guidelines based on a stricter interpreation of the Cohen's Kappa coefficient,[@mchugh2012] presented in Table \@ref(tab:gwet-table).

Gwet's AC1 is calculated using the following:

-> Insert formula here. Need to be sure of how to calculate. [@gwet2008; @sim2005]

```{r gwet-table, echo = FALSE}
```

```{r agreement-table-inter, echo = FALSE}
```

```{r agreement-table-intra, echo = FALSE}
```
...
```{r prisma-flow, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="PRISMA flow diagram, detailing the movement of studies through the systematic review.", out.width="100%"}
```
