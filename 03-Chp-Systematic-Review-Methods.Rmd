...
I used a random-effects model in all meta-analysis presented in this chapter.[[@hedges1998;@dersimonian1986] Random-effects meta-analysis does not assume one true underlying effect, but rather allows for a distribution of true effects with variance $\tau^2$. The weight ($w$) assigned to each result (denoted as $y_i$) is then given as the inverse of variance of that result plus the estimate of between-result variance, denoted as $w_i = \frac{1}{v_i+\tau^2}$ for result $i$.
