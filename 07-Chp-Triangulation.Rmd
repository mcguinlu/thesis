...
In epidemiology, triangulation is the practice of using multiple sources of evidence to provide more reliable answers to a research question. Different sources of evidence will have different limitations. If the result from each source points towards the same answer, this improves our confidence in the conclusion. Triangulation can either be _qualitative_ or _quantitative_. In qualitative triangulation, the results of each evidence source are discussed. In contrast, quantitative triangulation aims to combine the different results mathematically to obtain one single overall result.
...
(ref:priorsAdd-caption) __Prior distributions mapped to different extents of bias.__ Prior values assigned to additive and proportional biases, defined as log-normal distributions with mean $\mu$ and variance $\sigma^2$ using the notation $N(\mu,\sigma^{2})$. For additive biases, two scenarios were defined (see main text). Note that proportional biases are defined on the log scale for ease of calculation, but are exponentiated prior to their use for adjustment. 
