...
In __Chapter \@ref(sys-rev-methods-heading)__, I described the methods of a comprehensive systematic review [and meta-analyses]{.correction} of all available evidence on the association of both lipid levels and lipid-regulating agents with dementia outcomes. The results of this analysis are then presented in __Chapter \@ref(sys-rev-results-heading)__.
...
The first is a large-scale analysis of 953,635 patients in the CPRD, followed from their first LDL-c measurement.[@iwagami2021] The study found evidence for a slightly increased risk of dementia with higher LDL-c levels at mid-life, driven by the finding for the Alzheimer's disease subgroup. These results are consistent with the findings presented in this thesis, particularly as this study did not have access to _Apo_$\mathcal{E}4$ status and so could not adjust for confounding by this variable. _Apo_$\mathcal{E}4$ is a key risk factor for both Alzheimer's disease and raised LDL-c levels,[@bennet2007] and so could explain the harmful association observed. This is supported by a simulation analysis reported within this study, which used published information to predict that _Apo_$\mathcal{E}4$ would induce a risk ratio of 1.09 per 1-SD increase in LDL-c in the absence of a true causal effect.
...
[Fourthly, despite their wide use in the existing literature (as illustrated in the systematic review in Chapters]{.correction} \@ref(sys-rev-methods-heading) [&]{.correction} \@ref(sys-rev-results-heading) [), the clinical validity of dementia subtypes (Alzheimer's disease, vascular dementia, etc.) could be questioned and may lead to spurious results. This is supported by the difficulty in diagnosing a specific dementia subtype, and findings from population-based autopsies samples suggesting that "pure" Alzheimer's/vascular dementia outcomes are rare when compared to "mixed" dementia outcomes in older age groups, as discussed in detail in the Introduction (see Section]{.correction} \@ref(underlying-pathologies)[). In this case, results for the all-cause dementia outcome may be the most clinically useful, but this outcome was not the primary focus of the final chapters of these thesis.]{.correction}
