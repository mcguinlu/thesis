...
It has been widely used in the literature, and is comparable to Bradford-Hill's criteria of ``consistency'', that is the replication of an observed relationship across several different contexts.\textsuperscript{\protect\hyperlink{ref-hill1965}{314}} A good working definition of the triangulation is:
...
For example, while it may be true that all non-randomised studies share some \emph{minimal level} of bias, investigation of the biases applicable to each individual result gives useful additional information, and is essential for any attempt at quantitative triangulation.
...
In contrast, \emph{quantitative triangulation} attempts to obtain a overall numerical result by synthesising the different types of evidence. This method is in its infancy, given important methodological challenges that are illustrated in this Chapter.
...
\caption[Summary of research designs included in this thesis]{\label{tab:thesisOverview-table}Summary of studies included in this thesis, and used as evidence sources in the triangulation exercise. Note, Chapter \ref{sys-rev-tools-heading} is intentionally not included in this table, as it describes a tool rather than a research study.}
\centering
...
In addition to the qualitative discussion of evidence, I attempted to integrate the numerical results of the multiple approaches. This proof-of-concept approach incorporated advancements in the way that bias in results is assessed (domain-based assessment tools) and existing methods for bias-adjusted meta-analysis\textsuperscript{\protect\hyperlink{ref-turner2009}{317}} to to illustrate how causal questions could be addressed under this quantitative triangulation framework.
...
\item
  Identify of relevant evidence sources
\item
...
\item
  Perform bias-adjusted meta-analysis using bias-adjusted results
\end{enumerate}
...
The risk of bias tools used to assess each result are described in detail in Section \ref{risk-of-bias}. In summary, randomized controlled trials were assessed using the RoB 2 tool;\textsuperscript{\protect\hyperlink{ref-sterne2019}{145}} non-randomised studies of interventions (NRSI), were assessed using the ROBINS-I tool;\textsuperscript{\protect\hyperlink{ref-sterne2016}{142}}

For the sake of consistency, I mapped the different acceptable judgements in each risk-of-bias tool to a harmonised set of judgements: ``Low'', ``Moderate'',``High''. The exact mapping can be seen in Table \ref{tab:robLevelsMapping-table}. Of note, no mapping was performed for the critical risk-of-bias levels present in the tools for non-randomised studies. This is because current best practice in evidence synthesis is to exclude all studies at critical risk of bias for further quantitative synthesis.
...
Helper functions in the forthcoming \texttt{triangulate} package - see Appendix @ref() - help to preprocess risk of bias data for use with both bias-direction plots and adjusted meta-analyses. These bias direction plots themselves were created using the risk of bias isualisation tool described in Appendix \ref().
...
\caption[Additive bias distributions]{\label{tab:priorsAdd-table}Distributions for additive bias assigned to each bias judgement under two sensitivity scenarios, on the log scale.}
\centering
\begin{tabular}[t]{>{}lcc}
\toprule
\textbf{Bias Level} & \textbf{Scenario 1} & \textbf{Scenario 2}\\
\midrule
\textbf{Low} & - & -\\
\midrule
\textbf{Moderate} & N(.08, .05) & N(.08, .05)\\
\midrule
\textbf{High} & N(.16, .12) & N(.4, .29)\\
\bottomrule
\end{tabular}
\end{table}
...
Reasonable values for the position and spread of the distributions were informed by calculating the mean and maximum adjustment values as defined by the original elicitation exercise (see Appendix @ref(?) for more details on how these were calculated).\textsuperscript{\protect\hyperlink{ref-turner2009}{317}}
...
Once each individual result has been adjusted, they are combined n a standard random-effects meta-analysis

~

\hypertarget{tri-software}{%
\subsubsection{Software}\label{tri-software}}

Personal communication with the authors of the original method resulted in the STATA code to implement the

As detailed in the introduction to this section, the model for defining bias-indirectness-adjusted results was available as STATA code. The has since been generalised as part of the \texttt{triangulate} R package to allow for users to apply the approach detailed here.
...
All meta-analyses conducted within the package are implemented using the \texttt{metafor} R package.
...
\subsubsection{Causal question \#1}\label{causal-question-1}}
...
\begin{figure}[H]
\includegraphics[width=1\linewidth]{figures/tri/midlife_AD} \caption[Bias direction plot, summarising internal biases related to the]{\textbf{Bias direction plot, summarising internal biases related to effect of LDL-c on Alzheimer's disease :}}\label{fig:ldlAdBiasDirection}
\end{figure}
...
\subsubsection{Causal question \#2}\label{causal-question-2}}
...
\begin{figure}[H]
\includegraphics[width=1\linewidth]{figures/misc/UoBcrest} \caption[Effect of LDL-c on All-cause dementia in late life]{\textbf{Effect of LDL-c on All-cause dementia in late life:}}\label{fig:ldlDemBiasDirection}
\end{figure}
...
\subsubsection{Problems with quantitative triangulation approach}\label{problems-with-quantitative-triangulation-approach}}
...
Elicitation and bias-adjusted not as favoured versus weighting\textsuperscript{\protect\hyperlink{ref-stone2020}{320}} though weighting at the overall risk-of-bias level loses some of the information contained in domain-level based assessments.

No good information on what the potential distributions for different levels of bias (High vs Moderate vs Low) should be.

Meta-epidemiological studies on the effect of different methodological issues on results exist for randomised controlled trials,\textsuperscript{\protect\hyperlink{ref-amer2021}{321},\protect\hyperlink{ref-page2016}{322}} suggesting that X, Y and Z.
...
Appreciation that both the risk of bias and indirectness of a result should be assessed is growing. Indeed, some existing risk of bias for the assessment of diagnostic test accurary\textsuperscript{\protect\hyperlink{ref-whiting2011a}{323}} and prediction models\textsuperscript{\protect\hyperlink{ref-moons2019}{324}} consider indirectness (termed applicability in these tools) alongside the assessment of bias.

~

\hypertarget{subjectivity-of-risk-of-bias-assessments}{%
\subsubsection{Subjectivity of risk of bias assessments}\label{subjectivity-of-risk-of-bias-assessments}}
...
\hypertarget{defining-appropriate-values-for-biasindirectness-adjustment}{%
\subsubsection{Defining appropriate values for bias/indirectness adjustment}\label{defining-appropriate-values-for-biasindirectness-adjustment}}
...
Like other studies examining the prior distributions of heterogeneity across different topic areas

Talk about different ways to define bias terms, using expert-elicitation vs .\textsuperscript{\protect\hyperlink{ref-rhodes2020}{326}}
...
\hypertarget{exclusion-of-studies-at-critical-risk-of-bias}{%
\subsubsection{Exclusion of studies at critical risk of bias}\label{exclusion-of-studies-at-critical-risk-of-bias}}
...
\hypertarget{type-of-bias-proportional-vs-additive-not-shown-on-bias-direction-plot}{%
\subsubsection{Type of bias (proportional vs additive) not shown on bias direction plot}\label{type-of-bias-proportional-vs-additive-not-shown-on-bias-direction-plot}}
...
\subsubsection{Challenges of real-life data}\label{challenges-of-real-life-data}}
...
\subsection{Related approaches}\label{related-approaches}}

~

\newpage

\hypertarget{need-for-new-method}{%
\subsection{Need for new method}\label{need-for-new-method}}
...
\hypertarget{filling-in-the-gaps}{%
...
Found slight raised risk of dementia in CPRD patients () with higher LDL-c measured at midlife, driven by the finding for Alzheimer's disease.\textsuperscript{\protect\hyperlink{ref-iwagami2021}{327}} Not inconsistent with the findings presented in this thesis
...
Need for large simple trials for common disease where small treatment effect can have large effect -\textsuperscript{\protect\hyperlink{ref-yusuf1984}{328}}
...
\leavevmode\hypertarget{ref-rhodes2020}{}%
\CSLLeftMargin{326. }
\CSLRightInline{Rhodes, K. M. \emph{et al.} Adjusting trial results for biases in meta-analysis: Combining data-based evidence on bias with detailed trial assessment. \emph{Journal of the Royal Statistical Society. Series A, (Statistics in Society)} \textbf{183}, 193--209 (2020).}
...
All projects in these thesis attempt to conform to minimal best practices for research computing.\textsuperscript{\protect\hyperlink{ref-wilson2014}{329},\protect\hyperlink{ref-wilson2017}{330}}
